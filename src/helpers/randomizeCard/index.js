...
class Card {
	...
	generate() {
		while (true) {
			...
			const reroll =
				...
				(this.type == 'Spell' && random(0,3) > this.effCost)
	...
	getEffect() {
		...
		if (this.type == 'Spell' && this.ability.toLowerCase().includes('convert')) {this.effCost += 2}
		if (this.type == 'Structure' && this.ability.toLowerCase().includes('destroy this structure')) {this.effCost -= 2}
	...
	fillSlots() {
		while (true) {
			...
			console.log(validSlots, target, this.faction)
