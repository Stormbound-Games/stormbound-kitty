...
class Card {
	...
	getRace() {
		...
		if (this.faction === 'any') {
			this.faction = arrayRandom(['neutral', 'shadowfen', 'ironclad', 'winter', 'swarm'])
		}
	...
	getEffect() {
		...
		if (this.ability.startsWith('On play') && this.race.name === 'dragon' && random(1, 2) === 1 && this.movement == 0) {
			this.ability = this.ability.replace('On play, ',  'On play, fly to {spaceSpell} and ')
		}
		...
		} else if(this.ability.startsWith('Before attacking') && this.ability.includes('{targetUnit} enemy {targetUnit2}')  && random(1, 2) === 1) {
			this.ability = this.ability.replace('Before attacking', 'Before attacking an enemy unit')
			this.ability = this.ability.replace('{targetUnit} enemy {targetUnit2}', 'it')
			this.effCost *= 0.75
		}
	
		if (this.race.name === 'feline' && this.ability.includes('{felineCondPlay}')) {
